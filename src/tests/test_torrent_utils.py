...
def test_extract_infohash_from_raw_hash():
    assert extract_infohash(INFOHASH.upper()) == INFOHASH


def test_extract_infohash_from_peerflix_realdebrid_url():
    url = (
        "https://addon.peerflix.mov/realdebrid/provider-token/"
        f"{INFOHASH}/null/0/El%20arma%20del%20enga%C3%B1o%20BD1080.mkv"
    )

    assert extract_infohash(url) == INFOHASH


def test_realdebrid_token_is_not_treated_as_infohash():
    url = (
...
def test_hash_embedded_in_filename_is_not_treated_as_infohash():
    assert (
        extract_infohash(f"https://example.com/video-{INFOHASH}.mkv") is None
    )
