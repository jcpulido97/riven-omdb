...
This fork uses OMDb for movie, show, season, and episode release dates. The
bundled key can be replaced by setting `OMDB_API_KEY`; every OMDb request adds
the configured key automatically. TMDB and TVDB remain fallbacks when OMDb has
no usable release date, and the optional Trakt content service remains
